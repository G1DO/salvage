...
| `run <path> [--backup] [--expected-table] [--run-id] [--run-dir] [--artifact]` | exit 0 JSON `verified` + cleanup ok | exit 1 typed verdict (`restore/...`, `app/...`, `isolation/...`, `timed-out`, `cancelled`, run-state) | exit 2 `io`/usage; `--artifact` on v1 → `usage` |
